...
class FREnvelope {
    constructor(path) {
        ...
        this.entry = {};
        this.version = 2;
        this.fp = new PFileStream(fs.openSync(path, "r"), PageSize);
    }
...
class StrReader {
    ...
    read(fd, offset) {
        let n = this.nr.read(fd, offset);
        let buffer = fd.readSync(offset + 8, n);
...
class StructView {
    ...
    toString() {
        ...
        for(let f in this.fields) {
            if(Array.isArray(this.fields[f])) {
                r += `${f} = [${this.fields[f]}], `;
            } else {
                r += `${f} = ${this.fields[f]}, `;
            }
...
class DateTimeReader {
  constructor(renv, ty, repty) {
    this.nr = makeReader(renv, repty);
  }

  read(fd, offset) {
    return new Date(this.nr.read(fd, offset)/1000000.0);
  }
}

class UuidReader {
    constructor(renv, ty, repty) {
        this.nr = new FArrReader(renv, Prim("byte"), 16)
    }

    read(fd, offset) {
        return this.nr.read(fd, offset);
    }
}
...
function makeCustomReader(name, renv, ty, repty) {
    ...
    if (mkR)
        ...
        return new CommonReader(renv, ty, repty);
}
...
addReaderType("datetime", (renv, ty, repty) => new DateTimeReader(renv,ty, repty));
addReaderType("uuid", (renv, ty, repty) => new UuidReader(renv,ty, repty));
