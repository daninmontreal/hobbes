...
function readUint64(buffer, offset) {
    let n = buffer.readUIntLE(offset, 6);
    return n;
}
...
class PFileStream {
    ...
    readSync(pos, size) {
        ...
        let bytes = fs.readSync(this.fd, buffer, 0, size, pos);
        if (bytes != size) {
            fail(`fail to read ${bytes} != ${size}`);
        }
        return buffer;
    }
}
...
class UnpackReader {
    ...
    read(fd, offset) {
        let buffer = fd.readSync(offset, this.sz); 
        if(this.fmt == "?") { //bool
